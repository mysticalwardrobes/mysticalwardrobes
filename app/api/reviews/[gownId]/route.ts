...
import { sanityClient, REVIEWS_BY_GOWN_QUERY } from '@/lib/sanity';
import { Review } from '../model';

export async function GET(
  request: Request,
  { params }: { params: Promise<{ gownId: string }> }
) {
  try {
    const { gownId: filterGownId } = await params;
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🔍 REVIEWS BY GOWN API REQUEST');
    console.log(`   Gown ID: ${filterGownId}`);
    console.log(`   Timestamp: ${new Date().toISOString()}`);

    // Fetch reviews from Sanity filtered by gown ID
    const fetchStart = Date.now();
    const sanityResponse = await sanityClient.fetch<Review[] | null>(
      REVIEWS_BY_GOWN_QUERY,
      { gownId: filterGownId }
    );
    ...
    if (!sanityResponse || !Array.isArray(sanityResponse)) {
      console.log('⚠️ No reviews found for this gown');
